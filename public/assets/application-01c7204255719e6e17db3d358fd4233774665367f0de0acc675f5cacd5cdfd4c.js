...
(function() {
  $(function() {
    return $('.del-subform-button').click(function() {
      if (confirm("Are you sure that you want to remove this item?")) {
        $(this).prevAll('input[type="hidden"]').val(true);
        return $(this).closest('div.form-set').hide();
      }
    });
  });

  $(function() {
    return $('.add-subform-button').click(function() {
      var $template_clone, $template_object, no_form_sets;
      no_form_sets = $('.form-set').length;
      $template_object = $(this).closest('div.form-group').prev();
      $template_clone = $template_object.clone(true);
      $template_clone.find(':input').each(function() {
        var new_id, new_name, old_id, old_name;
        if ($(this).attr('id')) {
          old_id = $(this).attr('id');
        ...
        return $(this).attr('name', new_name);
      });
      $template_clone.show();
      return $template_object.before($template_clone);
    });
  });
...
(function() {
  $(function() {
    return $('.field_with_errors').closest('div.form-group').addClass('has-error');
  });

  $(function() {
    return $('.show-filter-row').click(function() {
      var r;
      r = $('.filter-row');
      r.toggle();
      if (r.is(':hidden')) {
        return r.closest('form').get(0).reset();
      }
    });
  });
  ...
  $(function() {
    ...
    return $.fn.datepicker.defaults.showOnFocus = false;
  });
...
//
// turbolinks disabled for the time being; when necessary, the gem
// jquery.turbolinks could/should be included to resolve problems with js
// functions attached to add/del-subform-buttons (2014-12-02 WR)
