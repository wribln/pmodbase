...
class CountryNameTest < ActiveSupport::TestCase
  ...
  test 'all scopes' do
    as = CountryName.as_abbr( 'GER' )
    assert_equal 1, as.length

    as = CountryName.as_abbr( 'XYZ' )
    assert_equal 0, as.length

    as = CountryName.ff_code( 'GER' )
    assert_equal 1, as.length

    as = CountryName.ff_code( 'XYZ' )
    assert_equal 0, as.length

    as = CountryName.as_desc( 'States' )
    assert_equal 1, as.length

    as = CountryName.as_desc( 'foobar' )
    assert_equal 0, as.length

    as = CountryName.ff_label( 'States' )
    assert_equal 1, as.length
